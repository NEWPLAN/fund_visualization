...
def get_all_stock(type: str = "sh"):
    ...
    assert type in allow_stock, "Invalid type={}, must be one of {}...".format(
        type, "/".join(allow_stock)
    )
    import akshare as ak
    ...
    if stock_zh_a_spot_em_df is None:
        ...
        return None
    logger.info(f"stock header:\n{stock_zh_a_spot_em_df.columns}")
    # Print basic info (optional)
    logger.info(
        f"Fetched {len(stock_zh_a_spot_em_df)} stocks as: \n{stock_zh_a_spot_em_df[['代码', '名称']].head()}"
    )

    # just return the code and name of stocks
    # Convert to list of tuples (code, name) without index
    stock_list = (
        stock_zh_a_spot_em_df[["代码", "名称"]].to_records(index=False).tolist()
    )
    return stock_list
